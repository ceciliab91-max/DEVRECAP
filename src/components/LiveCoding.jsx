...
export default function LiveCoding({ onGoToProfile }) {
  ...
  const getSubjectBadge = (sub) => {
    switch (sub) {
      case 'CSS': return 'bg-blue-500/10 text-blue-400 border-blue-500/20';
  ...
  return (
    <div className="space-y-8 pb-16 animate-fadeIn">
      ...
      {/* Hero Header */}
      <div className="relative overflow-hidden rounded-3xl bg-slate-800/80 border border-slate-700 p-6 sm:p-8 shadow-xl space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-2 px-4 py-2 rounded-2xl bg-slate-900 border border-slate-700 text-xs font-semibold text-cyan-400">
          ...
          {filteredChallenges.map((item, idx) => (
            <button
              ...
              className={`px-4 py-2.5 rounded-2xl text-xs font-bold border transition-all whitespace-nowrap ${selectedIdx === idx
                  ? 'bg-cyan-500/20 text-cyan-300 border-cyan-500/50 shadow-md shadow-cyan-500/10 font-extrabold ring-1 ring-cyan-400'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-5 flex flex-col justify-between p-6 sm:p-7 rounded-3xl bg-slate-800/70 border border-slate-700 shadow-xl">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                className="w-full p-4 rounded-2xl bg-slate-950 border border-slate-800 text-indigo-300 font-mono text-xs focus:outline-none focus:ring-2 focus:ring-cyan-500/50 transition-all leading-relaxed shadow-inner"
          ...
          <div className="space-y-3 pt-2">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                ...
                className="flex-1 py-3 px-4 rounded-2xl bg-slate-700 hover:bg-slate-600 text-white font-bold text-xs flex items-center justify-center space-x-2 transition-all"
              >
            ...
            <button
              ...
              className="w-full py-2.5 px-4 rounded-2xl bg-slate-900 hover:bg-slate-700 text-slate-400 hover:text-white border border-slate-700 font-semibold text-xs transition-all flex items-center justify-center space-x-1.5"
            >
        ...
        <div className="space-y-6 flex flex-col justify-between">
          ...
          {/* AI Loader Indicator */}
          {isAiLoading && (
            <div className="p-8 rounded-3xl bg-slate-800/80 border border-purple-500/30 shadow-xl text-center space-y-4 animate-pulse">
              <Loader2 className="w-10 h-10 text-purple-400 animate-spin mx-auto" />
              <div>
                <h4 className="text-base font-bold text-white">L'esaminatore IA sta correggendo la tua prova...</h4>
          ...
          {aiVerdict && !isAiLoading && (
            <div className={`p-6 rounded-3xl border transition-all space-y-4 shadow-2xl animate-fadeIn ${aiVerdict.isSuccess
                ? 'bg-gradient-to-b from-emerald-950/60 via-slate-800/80 to-slate-900 border-emerald-500/50'
                : 'bg-gradient-to-b from-red-950/60 via-slate-800/80 to-slate-900 border-red-500/50'
              }`}>
              ...
              <div className="flex items-center justify-between pb-3 border-b border-slate-700/80">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <span className="text-xs font-bold text-purple-300 block">{aiVerdict.profName}</span>
                    <span className="text-[10px] text-slate-400">{aiVerdict.role}</span>
                  </div>
                </div>

                <div className="text-right">
                  <span className="text-xs font-semibold text-slate-400 block uppercase tracking-wider">Voto Assegnato</span>
                  <span className={`text-xl sm:text-2xl font-black ${aiVerdict.isSuccess ? 'text-emerald-400' : 'text-red-400'}`}>
                    {aiVerdict.voto}
                  </span>
                </div>
              </div>
          ...
          {evalResult && !aiVerdict && (
            <div className={`p-5 rounded-3xl border transition-all space-y-3 ${evalResult.isSuccess
              ...
              <div className="space-y-1 pt-1 text-xs">
                {evalResult.passed.map((rule, i) => (
                  <div key={i} className="flex items-center space-x-2 text-emerald-400 font-semibold">
                    <CheckCircle2 className="w-3.5 h-3.5" />
                    <span>Regola osservata: {rule}</span>
                  </div>
                ))}
                {evalResult.failed.map((rule, i) => (
                  <div key={i} className="flex items-center space-x-2 text-red-400 font-semibold">
          ...
          {!aiVerdict && (
            <div className="p-5 rounded-3xl bg-slate-800/70 border border-slate-700 shadow-xl space-y-3">
              <div className="flex items-center justify-between pb-2 border-b border-slate-700 text-xs font-semibold text-slate-400">
          ...
          {showSolution && (
            <div className="p-5 rounded-3xl bg-indigo-950/40 border border-indigo-500/30 space-y-2 animate-fadeIn">
