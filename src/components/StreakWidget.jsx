...
export default function StreakWidget({ onActivityRecorded }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl bg-white dark:bg-slate-900 border border-amber-500/30 dark:border-amber-500/30 p-6 shadow-sm transition-all hover:border-amber-500/50">
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center space-x-4">
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-md transition-transform duration-300 ${
            isStudiedToday
            ...
            <Flame className={`w-8 h-8 ${isStudiedToday ? 'text-amber-200 fill-amber-300' : 'text-slate-400 dark:text-slate-400'}`} />
          </div>
          ...
          <div>
            <div className="flex items-center space-x-2">
              ...
              {isStudiedToday && (
                <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-purple-50 text-purple-700 border border-purple-200 dark:bg-purple-950/40 dark:text-purple-300 dark:border-purple-800 flex items-center gap-1">
        ...
        {!isStudiedToday && (
          <button
            ...
            className="self-start md:self-center px-4 py-2.5 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 text-white text-xs font-extrabold shadow-md shadow-orange-500/20 flex items-center space-x-2 transition-all hover:scale-105"
          >
            ...
            return (
              ...
              >
                <span className={`text-[11px] font-bold block ${
                  day.isToday ? 'text-indigo-600 dark:text-indigo-400 font-black' : 'text-slate-600 dark:text-slate-400'
                }`}>
                  {day.name}
                </span>
                ...
                <div className="mt-2 flex items-center justify-center">
                  {hasStudied ? (
                    <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-amber-500 to-orange-500 text-white flex items-center justify-center shadow-sm shadow-orange-500/30">
                      <Check className="w-3.5 h-3.5 stroke-[3]" />
                    </div>
                  ) : day.isToday ? (
                    <div className="w-6 h-6 rounded-full border-2 border-indigo-500/50 dark:border-indigo-400/50 border-dashed flex items-center justify-center text-indigo-600 dark:text-indigo-300 text-[10px] font-bold animate-pulse">
                      •
                    </div>
                  ) : (
                    <div className="w-6 h-6 rounded-full bg-slate-200 dark:bg-slate-700/60 border border-slate-300 dark:border-slate-600" />
                  )}
                </div>
