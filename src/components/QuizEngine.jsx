...
export default function QuizEngine({ questions, modeInfo, onFinishQuiz, onCancelQuiz }) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-16">
      ...
      {/* Top Header Bar */}
      <div className="p-4 sm:p-5 rounded-2xl bg-slate-800/80 dark:bg-slate-800/80 light:bg-white border border-slate-700 dark:border-slate-700 light:border-slate-200 shadow-md space-y-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className={`flex items-center space-x-2 px-3 py-1.5 rounded-xl border text-xs font-mono font-bold transition-all ${
              initialTimerSecs > 0 && timeLeft < 300 
                ...
                : 'bg-slate-900/60 text-indigo-400 border-slate-700'
            }`}>
            ...
            >
              {flagged[currentQ.id] ? <BookmarkCheck className="w-4 h-4 text-amber-400" /> : <Bookmark className="w-4 h-4" />}
            ...
            <button
              ...
              className="p-2 rounded-xl bg-slate-900/60 text-slate-300 border border-slate-700 hover:bg-slate-700 transition-colors"
      ...
      {showDrawer && (
        <div className="p-5 rounded-2xl bg-slate-800/90 border border-slate-700 space-y-3 animate-fadeIn">
          ...
          <div className="grid grid-cols-6 sm:grid-cols-10 md:grid-cols-12 gap-2">
            {questions.map((q, idx) => {
              ...
              if (isAnswered) btnClass = 'bg-emerald-500/20 text-emerald-400 border-emerald-500/40 font-bold';
              if (isFlagged) btnClass = 'bg-amber-500/20 text-amber-400 border-amber-500/40 font-bold';
              if (isCurrent) btnClass += ' ring-2 ring-indigo-400 border-indigo-400';
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-slate-800/60 dark:bg-slate-800/60 light:bg-white border border-slate-700/80 dark:border-slate-700/80 light:border-slate-200 shadow-xl space-y-6">
        
        {/* Question Text */}
        <div className="space-y-3">
          <h2 className="text-lg sm:text-xl font-bold text-white dark:text-white light:text-slate-900 leading-snug">
            {currentQ.question}
          </h2>
        </div>
            ...
            return (
              <button
                ...
                className={`w-full text-left p-4 rounded-2xl border transition-all duration-200 flex items-center justify-between group ${
                  isSelected
                    ? 'bg-indigo-600/20 border-indigo-500 text-indigo-200 font-semibold shadow-md shadow-indigo-600/10'
                    : 'bg-slate-900/40 dark:bg-slate-900/40 light:bg-slate-50 border-slate-700/70 dark:border-slate-700/70 light:border-slate-200 text-slate-300 dark:text-slate-300 light:text-slate-700 hover:bg-slate-700/40 hover:border-slate-600'
                }`}
              >
                <div className="flex items-center space-x-4">
                  <span className={`w-8 h-8 rounded-xl font-bold text-xs flex items-center justify-center transition-colors ${
                    isSelected
                      ...
                      : 'bg-slate-800 dark:bg-slate-800 light:bg-slate-200 text-slate-400 group-hover:text-slate-200'
                  }`}>
                    {keyLetters[idx]}
                  </span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-md p-6 rounded-3xl bg-slate-900 border border-slate-800 shadow-2xl space-y-5">
            ...
            <div className="space-y-2">
              <h3 className="text-xl font-bold text-white">Confermi l'invio dell'esame?</h3>
              <p className="text-xs text-slate-300 leading-relaxed">
                Hai risposto a <strong className="text-indigo-400">{answeredCount}</strong> domande su <strong className="text-white">{totalQ}</strong> complessive.
            ...
            <div className="flex items-center space-x-3 pt-2">
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-slate-800 text-slate-300 hover:bg-slate-700 text-xs font-semibold transition-all"
              >
                Torna al Test
              </button>
              <button
