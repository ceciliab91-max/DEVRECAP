    ...
    <div className="space-y-8 pb-16 animate-fadeIn">
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              {['ALL', 'CSS', 'JavaScript', 'React', 'SQL'].map((sub) => (
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-xl text-xs font-semibold border transition-all ${
                    subjectFilter === sub
                      ? 'bg-indigo-500/20 text-indigo-600 dark:text-indigo-300 border-indigo-500/40 font-bold'
            ...
            <button
              ...
              className="flex items-center space-x-1.5 text-xs text-slate-600 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-400 transition-colors"
            >
            ...
            <div className="max-w-2xl mx-auto space-y-6">
              ...
              <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400 font-medium">
                ...
                <div className="flex items-center space-x-2">
                  <span className="px-2.5 py-1 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-indigo-600 dark:text-indigo-400 font-bold shadow-sm">
                    {currentCard.subject} &bull; {currentCard.category}
                  </span>
                  {cardStatusMap[currentCard.id] === 'known' && (
                    <span className="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-[11px] font-bold border border-emerald-500/30">
                      La so ✅
                    </span>
                  )}
                  {cardStatusMap[currentCard.id] === 'review' && (
                    <span className="px-2 py-0.5 rounded bg-amber-500/20 text-amber-700 dark:text-amber-400 text-[11px] font-bold border border-amber-500/30">
                      Da rivedere 🔄
                    </span>
                  )}
                </div>
              ...
              <div
                ...
                className="cursor-pointer group relative min-h-[320px] rounded-3xl bg-white dark:bg-slate-900 border-2 border-purple-400/50 dark:border-purple-600/50 hover:border-purple-500 dark:hover:border-purple-500 p-8 shadow-md transition-all duration-300 flex flex-col justify-between"
              >
                {/* Top Hint */}
                <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                  ...
                  <span className="flex items-center space-x-1 text-slate-500 dark:text-slate-400 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                ...
                <div className="py-6 space-y-4">
                  {!isFlipped ? (
                    ...
                    <div className="space-y-4 text-center">
                      ...
                      <p className="text-slate-800 dark:text-slate-100 text-base sm:text-lg leading-relaxed max-w-lg mx-auto">
                        "{currentCard.front}"
                      </p>
                    </div>
                  ) : (
                    ...
                    <div className="space-y-4">
                      ...
                      {currentCard.codeSnippet && (
                        <div className="p-3.5 rounded-2xl bg-slate-950 text-indigo-300 border border-slate-800 font-mono text-xs overflow-x-auto shadow-inner">
              ...
              <div className="flex items-center justify-center gap-4">
                <button
                  ...
                  className="flex-1 max-w-[200px] flex items-center justify-center space-x-2 py-3 rounded-2xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30 text-xs font-bold transition-all shadow-sm"
                >
                ...
                <button
                  ...
                  className="flex-1 max-w-[200px] flex items-center justify-center space-x-2 py-3 rounded-2xl bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 border border-emerald-500/30 text-xs font-bold transition-all shadow-sm"
                >
              ...
              <div className="flex items-center justify-between pt-2">
                <button
                  ...
                  className="flex items-center space-x-1.5 px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 text-xs font-semibold transition-colors"
                >
      ...
      {subTab === 'mindmaps' && (
        <div className="space-y-6">
          ...
          {(() => {
            ...
            return (
              <div className="space-y-6 p-6 rounded-3xl bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 transition-colors">
                ...
                <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-2 shadow-sm">
                  <div className="flex items-center space-x-3">
                    ...
                    <h2 className="text-xl font-bold text-slate-900 dark:text-white">{activeMap.title}</h2>
                  </div>
                  <p className="text-xs text-slate-600 dark:text-slate-400">{activeMap.description}</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {activeMap.nodes.map((node, idx) => (
                    <div
                      key={node.id}
                      className="p-6 rounded-2xl bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 border-2 border-purple-400 dark:border-purple-600 shadow-sm space-y-4 flex flex-col justify-between"
                    >
                        ...
                        <ul className="space-y-2 text-xs text-slate-600 dark:text-slate-400">
                          {node.points.map((pt, ptIdx) => (
                            <li key={ptIdx} className="flex items-start space-x-2">
                              <ChevronRight className="w-3.5 h-3.5 text-indigo-500 dark:text-indigo-400 mt-0.5 flex-shrink-0" />
                      ...
                      {/* Exam Tip Footer Box */}
                      {node.tip && (
                        <div className="p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-[11px] text-amber-800 dark:text-amber-300 flex items-start space-x-2 font-medium">
      ...
      {subTab === 'notebooklm' && (
        <div className="max-w-3xl mx-auto space-y-8">
          ...
          <div className="p-8 sm:p-10 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-6 relative overflow-hidden">
            
            <div className="flex items-center space-x-4">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-blue-500 via-indigo-500 to-purple-500 flex items-center justify-center shadow-md">
                <FileText className="w-8 h-8 text-white" />
              </div>
              <div>
                <span className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider block">Integrazione Studio IA</span>
                <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Google NotebookLM</h2>
              </div>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-700/60 space-y-3">
              <h3 className="text-xs font-bold text-slate-900 dark:text-white flex items-center space-x-2">
                <FolderDown className="w-4 h-4 text-emerald-500 dark:text-emerald-400" />
            ...
            <div className="space-y-3 text-xs text-slate-600 dark:text-slate-300">
              <h4 className="font-bold text-slate-900 dark:text-white">Come Utilizzare NotebookLM:</h4>
            ...
            <div className="pt-2">
              <a
                ...
                className="inline-flex items-center justify-center space-x-3 px-6 py-3.5 rounded-2xl bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:opacity-95 text-white font-bold text-sm shadow-md shadow-indigo-600/30 transition-all transform hover:-translate-y-0.5"
              >
